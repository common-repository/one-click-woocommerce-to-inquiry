<?php
/*
Plugin Name: One click WooCommerce to Inquiry
Plugin URI:  https://www.vessno.com/
Description: This plugin will convert woocommerce shoping cart into inquiry.
Version:     1.2 
Author:      Ali Shan
...
function ocwti_get_woo_version_number() {
...
function ocwti_woo_custom_cart_button_text() {
    return __( 'Add to basket', 'woocommerce' );
}

// change view cart button text
function ocwti_changeViewCartButtonText( $translated_text, $text, $domain ){
    /**
     * Change text strings
     *
...
function ocwti_change_add_to_cart_msg($message, $product_id = null){
...
function ocwti_updating_url_for_gotoshop_btn(){
    /**
...
function ocwti_setInquiryPages($inquiryPageID = false){
    if($inquiryPageID){
...
function ocwti_init(){
    ...
    // Changing Add to Cart Button Text
    $version = ocwti_get_woo_version_number();
    if($version){
        $version = substr($version, 0, -2);

        if($version < 2.1){

            add_filter( 'add_to_cart_text', 'ocwti_woo_custom_cart_button_text' );    // < 2.1

        }else if ($version > 2.1){
            add_filter( 'woocommerce_product_add_to_cart_text', 'ocwti_woo_custom_cart_button_text' );    // 2.1 +
            add_filter( 'woocommerce_product_single_add_to_cart_text', 'ocwti_woo_custom_cart_button_text' );    // 2.1 +
        }
    }


    // Changing text of View Cart button.
    add_filter( 'gettext', 'ocwti_changeViewCartButtonText', 20, 3 );


    // Creating page
    $pageID = ocwti_creatingPages();

    // updating Go To Home button after inquiry
    add_filter( 'woocommerce_return_to_shop_redirect', 'ocwti_updating_url_for_gotoshop_btn' );

    // changing Added to cart text
    add_filter ( 'wc_add_to_cart_message', 'ocwti_change_add_to_cart_msg', 10, 2 );

    // Setting Up Inquiry pages to WooCommerce
    if($pageID)
        ocwti_setInquiryPages($pageID);
...
/*
function ocwti_printversion(){
    $result = ocwti_get_woo_version_number();
